...
; ---------------------------------------------------------------------------
QueueSlotCount = (v_vdp_comm_buffer_slot-v_vdp_comm_buffer)/DMAEntry.len
; ---------------------------------------------------------------------------
...
QueueStaticDMA macro src,length,dest
	...
	movea.w	(v_vdp_comm_buffer_slot).w,a1
	cmpa.w	#v_vdp_comm_buffer_slot,a1
	...
	move.w	a1,(v_vdp_comm_buffer_slot).w						; Write next queue slot
.done:
	if UseVIntSafeDMA==1
...
; ---------------------------------------------------------------------------
ResetDMAQueue macro
	move.w	#v_vdp_comm_buffer,(v_vdp_comm_buffer_slot).w
	endm
; ===========================================================================
...
.doubletransfer:
		...
		cmpa.w	#v_vdp_comm_buffer_slot-DMAEntry.len,a1	; Does the queue have enough space for both parts?
...
ProcessDMAQueue:
	movea.w	(v_vdp_comm_buffer_slot).w,a1
	jmp	.jump_table-v_vdp_comm_buffer(a1)
; ---------------------------------------------------------------------------
.jump_table:
	rts
	rept 6
