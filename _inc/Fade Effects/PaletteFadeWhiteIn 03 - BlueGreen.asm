; ---------------------------------------------------------------------------
; Subroutine to	fade in from white (Special Stage) -- Mode 03: Blue+Green (RetroKoH)
...
.checkcolors:
	...
	; Check components
		...
		bne.s	.debluegreen			; if yes, branch and decrement red to current colour
		cmpi.b	#$E,d1					; Does green need removal (yes if d1 is not $E)
		bne.s	.degreen				; if yes, branch and decrement green to current colour
	; ---------------------------------------------------------------------------
	; This time, blue and green will be reduced together on each frame, instead of separately
	; ---------------------------------------------------------------------------
; fallthrough
