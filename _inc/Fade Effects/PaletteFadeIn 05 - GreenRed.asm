; ---------------------------------------------------------------------------
; Subroutine to	fade in from black -- Mode 05: Green+Red (RetroKoH)
...
.fill:
		...
		move.w	#$15,d4		; fade-in will occur over 22 frames (decrease this?)
...
.checkcolors:
	...
	; Check components
		tst.b	d1						; Does blue need adding (yes if d5 is non-zero)
		bne.s	.addgreenred			; if yes, branch and add blue to current colour
		tst.b	d2						; Does red need adding (yes if d5 is non-zero)
		bne.s	.addred					; if yes, branch and add red to current colour
	; ---------------------------------------------------------------------------
	; This time, green and red will be added together on each frame, instead of separately
	; ---------------------------------------------------------------------------
		bra.s	.addblue
; ===========================================================================
...
.addgreenred:
		...
		tst.b	d2						; Does red also need adding (yes if d5 is non-zero)
